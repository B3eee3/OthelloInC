...
#include <string.h>
#include <ctype.h>
//all of the macros, function declarations, and struct definition
#include "othello.h"
//
...
void playGame()
{
    ...
    while(endGame == FALSE)
    {
        //sets end game equal to function isendgame passing in the board, and if endGame is true ends the game and stops the while loop
        ...
        // request the player's move and checks if a valid move can be made, and if not pass it to the other player
        ...
        //comented out previous increase loop variable
        //loop++;
    }

    //commented out since going into function gameOver
    //displayBoard(board);
...
//checks if there are valid moves in the horizontal
...
//checks if there are valid moves in the vertical
...
//checks if there are valid moves in the diagonal
...
//checks the results of the board and then updates it
...
//updates the board in the Horizontal direction
...
//updates the board in the Vertical direction
...
//updates the board in the Diagonal direction
...
//Checks if the game ends and then ends it
...
//counts up how many discs there are in the board
...
//checks if a valid move exists on the board
int validMoveExists(char board[ROW][COL], char playerChar)
{
    ...
    for(r=0;r<=MAX;++r)
    {
        for(c=0;c<=MAX;++c)
        {
            ...
            if(spaces > ZERO)
            {
                if(checkHorizontal == ZERO && checkDiagonal == ZERO)
                {
                    result = FALSE;
                }
                else
...
//prints out Game Over and displays the stats of the game  
